...
@connect(({ task, loading }) => ({
  newNodes: task.newInstanceNodes,
  graphNodeExpandLoading: loading.effects["task/fetchInstanceNode"],
}))
class GraphFlow4Instance extends Component {
  constructor(props){
    super(props);
    this.state = {
      graph: null,
  ...
  static getDerivedStateFromProps(nextProps, prevState){
    if(!nextProps.loading && prevState.graph && nextProps.graphDependencies 
      && (!isEqual(prevState.taskInstanceId,nextProps.taskInstanceId) || !isEqual(prevState.graphDependencies,nextProps.graphDependencies))){
      prevState.graph.read (nextProps.graphDependencies);
      return {
        ...prevState,
        graphDependencies: nextProps.graphDependencies,
        taskInstanceId: nextProps.taskInstanceId
      }
    }
...
export default GraphFlow4Instance